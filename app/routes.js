...
router.post('/contactos/:id',
    // Validaciones (mismas que para crear)
    body('nombre')
        .notEmpty()
        .withMessage('El nombre es obligatorio')
        .isLength({ min: 2, max: 255 })
    ...
    async (req, res) => {
        try {
            ...
            if (updated) {
                res.redirect(`/?message=${encodeURIComponent('Contacto actualizado exitosamente')}`);
            } else {
                res.redirect(`/?error=${encodeURIComponent('No se pudo actualizar el contacto')}`);
            }
        } catch (error) {
